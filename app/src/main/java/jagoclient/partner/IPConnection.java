...
//1A90 2015/03/01 try another session for wifidirect to fix test of connection delay//~1A90I~
//1A6D 2015/02/22 1A6C is not good(connection faile by unreachable),prepare button set on/off/ airplane mode on ip connection//~1A6DI~
//1A6C 2015/02/22 after WiFiDirect,standard IP Connection is too late. try WiFi Off before std IPConnection.//~1A6BI~
...
//1A05 2015/02/16 Asgts:2013/03/02 reject Accept/Connect if IP addr not avail//~1A05I~//~1A2jR~
//1A6A 2015/02/16 from Asgts                                       //~1A6AI~
//1A6n 2015/02/15 IP;identify connected device                     //~1A6nI~
//1A6k 2015/02/15 re-open IPConnection/BTConnection dialog when diconnected when dislog is opened.//~1A69I~
...
import com.Ahsv.awt.List;
import com.Ahsv.awt.ListData;
import com.Ahsv.java.FileOutputStream;
...
//  private ButtonAction btnDirect;                                //~1A6oI~//~1A90R~
//  private ButtonAction btnSetting;                               //~1A6DI~//~1A90R~
...
//      displaySession();                                          //~v101I~//~1A67R~
        ...
        displaySession();  //after PartnerList Created               //~1A67I~
...
//      btnDirect=                                                 //~1A6oI~//~1A90R~
//          new ButtonAction(this,0,R.id.WiFiDirectButton);	//change visibility from gone to visible//~1A65R~//~1A90R~
//      btnSetting=new ButtonAction(this,0,R.id.CallWiFiSetting);	//change visibility from gone to visible//~1A6DI~//~1A90R~
//      if (AG.osVersion<AG.ICE_CREAM_SANDWICH)  //android3        //~1A6oI~//~1A90R~
//      {                                                          //~1A6DI~//~1A90R~
//          btnDirect.setEnabled(false);                           //~1A6oI~//~1A90R~
//          btnSetting.setEnabled(false);                          //~1A6DI~//~1A90R~
//      }                                                          //~1A6DI~//~1A90R~
        setDismissActionListener(this/*DoActionListener*/);        //~@@@@I~//~v101I~
		validate();
		show();
        AG.aIPConnection=this;	//used when PartnerThread detected err//~1A6kR~
	}
    //******************************************                   //~1A6kR~
    public void updateViewDisconnected()                           //~1A6kR~
    {                                                              //~1A6kR~
    	acceptButton.setEnabled(true);                             //~1A6kR~
        acceptButton.setAction(R.string.AcceptIPConnection);       //~1A6kR~
        connectButton.setEnabled(true);                            //~1A6kR~
        connectButton.setAction(R.string.IPConnect);               //~1A6kR~
        gameButton.setEnabled(false);                              //~1A6kR~
...
//          else                                                   //~1A65I~//~1A90R~
//          if (o.equals(AG.resource.getString(R.string.WiFiDirectButton)))  //modal but no inputWait//~1A65I~//~1A90R~
//          {               //callback from Dialog after currentLayout restored//~1A65I~//~1A90R~
//              startWD();                                         //~1A65I~//~1A90R~
//          }                                                      //~1A65I~//~1A90R~
//          else                                                   //~1A6DI~//~1A90R~
//          if (o.equals(AG.resource.getString(R.string.CallWiFiSetting)))  //modal but no inputWait//~1A6DI~//~1A90R~
//          {               //callback from Dialog after currentLayout restored//~1A6DI~//~1A90R~
//              callWiFiSetting();                                 //~1A6DI~//~1A90R~
//          }                                                      //~1A6DI~//~1A90R~
        }                                                          //~3117I~
...
//                  Global.resourceString("Connection_to_")+c.Name,false);//~v101I~//~1A6BR~
    ...
    //******************************************                   //~v101R~
//  private void disconnectPartner()                               //~v101M~//~1A89R~
    public void disconnectPartner()                                //~1A89I~
    {                                                              //~v101M~
...
//        if (portNoWD!=0)                                         //~1A90R~
//            S=new Server(portNoWD,false);                        //~1A90R~
//        else                                                     //~1A90R~
...
//          {   PartnerList.remove(lc);                            //~v101I~//~1Af9R~
            {                                                      //~1Af9I~
                ...
                deleted_partner_prev=lc.previous();                //~1Af9I~
  				...
  				if (next!=null)                                    //~1Af9R~
					s=((Partner)(next.content())).Name; //selected after delete//~1Af9R~
    ...
    {                                                              //~1Af9I~
        ...
        {                                                          //~1Af9I~
			String s=((Partner)(deleted_partner.content())).Name;  //~1Af9R~
    ...
    {                                                              //~v101I~
        String l;
    	PL=new ListIP(this,R.id.PartnerList,R.layout.textrowlist);                                         //~1524R~//~v101R~
...
//            if (portNoWD!=0)                                     //~1A90R~
//                msg=AG.resource.getString(R.string.Msg_WaitingIPAccept,myIPAddr,portNoWD);//~1A90R~
...
//    //***********************************************************************//~1A65I~//~1A90R~
//    private void startWD()                                         //~1A65I~//~1A90R~
//    {                                                              //~1A65I~//~1A90R~
//        if (Dump.Y) Dump.println("IPConnection:startWD");          //~1A65I~//~1A90R~
//        try                                                        //~1A65I~//~1A90R~
//        {                                                          //~1A65I~//~1A90R~
//            aWDA=WDA.showDialog(this);                             //~1A65R~//~1A90R~
//        }                                                          //~1A65I~//~1A90R~
//        catch(Exception e)                                         //~1A65I~//~1A90R~
//        {                                                          //~1A65I~//~1A90R~
//            Dump.println(e,"IPConnection:startWD");                //~1A65I~//~1A90R~
//        }                                                          //~1A65I~//~1A90R~
//    }                                                              //~1A65I~//~1A90R~
//    //***********************************************************************//~1A65I~//~1A90R~
//    public void onCloseWDA()                                        //~1A67R~//~1A90R~
//    {                                                              //~1A65I~//~1A90R~
//        boolean owner=aWDA.WDAowner;                               //~1A65I~//~1A90R~
//        if (Dump.Y) Dump.println("IPConnection:closedWD owner="+owner);//~1A65I~//~1A90R~
//        try                                                        //~1A65I~//~1A90R~
//        {                                                          //~1A65I~//~1A90R~
//            String serveripa=aWDA.WDAipa;                           //~1A65I~//~1A90R~
//            String peername=aWDA.WDApeer;                        //~1A65I~//~1A67R~//~1A90R~
////            portNoWD=portNo+2;                                 //~1A90R~
//            if (!owner)                                            //~1A65I~//~1A90R~
//            {                                                      //~1A65I~//~1A90R~
//                if (Dump.Y) Dump.println("IPConnection:closedWD client server="+peername+",addr="+serveripa);//~1A65I~//~1A67R~//~1A90R~
////              String partnerprefix=AG.resource.getString(R.string.PartnerNamePeer)+"-";//~1A65I~//~1A6nR~//~1A90R~
//                String partnername=partnerprefix+peername;       //~1A65I~//~1A67R~//~1A90R~
//                Partner p=new Partner(partnername,serveripa,portNo,Partner.PRIVATE);//~1A65I~//~1A90R~
////                p=new Partner(partnername,serveripa,portNoWD,Partner.PRIVATE);//~1A90R~
//                updatePeer(p,partnerprefix);                       //~1A65I~//~1A90R~
//            }                                                      //~1A65I~//~1A90R~
//            else                                                   //~1A67I~//~1A90R~
//            {                                                      //~1A67I~//~1A90R~
//                if (Dump.Y) Dump.println("IPConnection:closedWD server server="+peername+",addr="+serveripa);//~1A67I~//~1A90R~
////              String partnerprefix=AG.resource.getString(R.string.PartnerNamePeer)+"-";//~1A67I~//~1A6nR~//~1A90R~
//                String partnername=partnerprefix+peername;         //~1A67I~//~1A90R~
//                Partner p=new Partner(partnername,WDCLIENT_IPPREFIX+serveripa,portNo,Partner.PRIVATE);//~1A67I~//~1A90R~
//                updatePeer(p,partnerprefix);                       //~1A67I~//~1A90R~
//            }                                                      //~1A67I~//~1A90R~
//        }                                                          //~1A65I~//~1A90R~
//        catch(Exception e)                                         //~1A65I~//~1A90R~
//        {                                                          //~1A65I~//~1A90R~
//            Dump.println(e,"IPConnection:closedWD");               //~1A65I~//~1A90R~
//        }                                                          //~1A65I~//~1A90R~
//    }                                                              //~1A65I~//~1A90R~
//    //***********************************************************************//~1A65I~//~1A90R~
//    private void updatePeer(Partner Ppartner,String Pprefix)       //~1A65I~//~1A90R~
//    {                                                              //~1A65I~//~1A90R~
//        Partner p;                                                 //~1A65I~//~1A90R~
//        boolean swupdate=false;                                    //~1A65I~//~1A90R~
//    //**************************                                   //~1A65I~//~1A90R~
//        p=pfind(Ppartner.Name);                                     //~1A67I~//~1A90R~
//        if (p!=null)                                               //~1A67I~//~1A90R~
//        {                                                          //~1A67I~//~1A90R~
//            p.Server=Ppartner.Server;                          //~1A65I~//~1A67R~//~1A90R~
//            p.Port=Ppartner.Port;                              //~1A65I~//~1A67R~//~1A90R~
//            swupdate=true;                                     //~1A65I~//~1A67R~//~1A90R~
//        }                                                          //~1A65I~//~1A90R~
//        if (!swupdate)                                             //~1A65R~//~1A90R~
//        {                                                          //~1A65I~//~1A90R~
//            PartnerList.prepend(new ListElement(Ppartner));        //~1A65I~//~1A90R~
//        }                                                          //~1A65I~//~1A90R~
//        updateplist();                                             //~1A65I~//~1A90R~
//        writeplist();   //update partner.cfg                       //~1A67I~//~1A90R~
//        if (!swupdate)                                             //~1A67I~//~1A90R~
//            PL.select(0);                                              //~1A65I~//~1A67R~//~1A90R~
//        else                                                       //~1A67I~//~1A90R~
//            setSelection(Ppartner.Name);                           //~1A67I~//~1A90R~
//    }                                                              //~1A65I~//~1A90R~
    //***********************************************************************//~1A67I~
    ...
    //**********************************************************************//~1A6nI~
        ...
        {                                                          //~1A6nI~
	        TextView tv;                                           //~1A6nI~
        //*******************                                      //~1A6nI~
            if (Dump.Y) Dump.println("ListIP:getViewCustom Ppos="+Ppos);
            tv=Pview;	                                           //~1A6nR~
            ListData ld=Plistdata;                                 //~1A6nR~
            if (chkDirect(ld,Ppos))                                //~1A6nR~
            {                                                      //~1A6nI~
//              if (Ppos!=Pselectedpos)                            //~1A6nR~
//                  tv.setBackgroundColor(connectedBGColor);       //~1A6nR~
	            tv.setTextColor(connectedLineColor.getRGB());               //~1A6nR~
            }                                                      //~1A6nI~
        }                                                          //~1A6nI~
        //***************************************                  //~1A6nI~
        private boolean chkDirect(ListData Pld,int Ppos)           //~1A6nR~
        {                                                          //~1A6nI~
            if (Pld.itemtext.startsWith(partnerprefix))            //~1A6nR~
            {                                                      //~1A6nI~
                Partner p=pfind(Pld.itemtext);                     //~1A6nR~
                if (p!=null)                                       //~1A6nI~
                {                                                  //~1A6nI~
...
//    //***********************************************************************//~1A6DI~//~1A90R~
//    private void callWiFiSetting()                                 //~1A6DI~//~1A90R~
//    {                                                              //~1A6DI~//~1A90R~
//        if (Dump.Y) Dump.println("IPConnection:callWiFiSetting");  //~1A6DI~//~1A90R~
//        AG.activity.startActivity(new Intent(Settings.ACTION_WIRELESS_SETTINGS));//~1A6DI~//~1A90R~
//    }                                                              //~1A6DI~//~1A90R~
}

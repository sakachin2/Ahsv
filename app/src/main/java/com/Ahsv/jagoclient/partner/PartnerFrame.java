...
//****************************************************************************//~@@@1I~
package com.Ahsv.jagoclient.partner;                               //~v107R~
...
import com.Ahsv.jagoclient.partner.PartnerThread;                  //~v107R~
...
//import com.Ahsv.rene.viewer.Viewer;                              //~1318I~//~v107R~
...
//  	super(name+Premotedevicename,serving,jagoclient.partner.PartnerFrame.CONN_TITLE_BT);//~1A8iI~//+1AecR~
		...
		if (Dump.Y) Dump.println("Ahsv:@@PartnerFrame@@ name="+name);//~@@@2I~
...
//        remoteDeviceName=Premotedevicename;                        //~v107I~
		...
		if (Dump.Y) Dump.println("Ahsv:PartnerFrame constructor namd="+name+",remote="+Premotedevicename+",local="+Plocaldevicename);//~v107R~
//        changeButton();                                            //~@@@@I~//~@@@2R~
	}
//****************************************************************************//~@@@@I~
...
//****************************************************************************//~v107I~
	...
	{                                                              //~v107R~
		if (Dump.Y) Dump.println("Ahsv:PartnerFrame connect localdevicename="+localDeviceName);//~v107R~
		try
...
//  	{   return false;                                          //~v107R~
    	{                                                          //~v107I~
            Dump.println(e,"Ahsv:PartnerFrame:connect");           //~v107R~
            return false;                                          //~v107I~
		}
...
//  public void open (Socket server)                               //~v107R~
	...
	{	if (Dump.Y) Dump.println("Starting partner server");       //~@@@1R~
		Server=server;
	    if (Dump.Y) Dump.println("Ahsv:PartnerFrame:open BTSocket="+Server);//~v107R~
		try
		{	Out=new PrintWriter(
				new DataOutputStream(Server.getOutputStream()),true);
			In=new BufferedReader(new InputStreamReader(
...
//        Out.close();                                             //~1AbMR~
//        if (Dump.Y) Dump.println("Asgts:PartnerFrame:doclose Out closed after sent @@@@end");//~1AbMI~
////      new CloseConnection(Server,In);                            //~v107R~//~1AbMR~
//        if (Dump.Y) Dump.println("Asgts:PartnerFrame:sleep before close IN");//~1AbMI~
//        if (In!=null)                                              //~v107I~//~1AbMR~
//            In.close();                                            //~v107I~//~1AbMR~
//        if (Server!=null)                                          //~v107I~//~1AbMR~
//        {                                                          //~v107I~//~1AbMR~
//            if (Dump.Y) Dump.println("Ahsv:PartnerFrame:doclose close BTSocket="+Server);//~v107R~//~1AbMR~
...
//        super.doclose();                                         //~1AbMR~
      ...
      {                                                            //~v107I~
      	Dump.println(e,"Ahsv:partnerFrame:doclose");               //~v107R~
      }                                                            //~v107I~
	}
//***************************************************************  //~@@@2I~

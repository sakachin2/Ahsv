//*CID://+1Ad9R~:                             update#=   15;       //~1Ad9R~
//***************************************************************************//~1A6AI~
//1Ad9 2015/07/21 additional to 1Ad6(OutOfMemory)                  //~1Ad9I~
//1Ad8 2015/07/21 (Asgts)//1A4h 2014/12/03 catch OutOfMemory       //~1Ad8I~
...
import com.Ahsv.awt.MemoryImageSource;                             //~@@@@R~
...
//*****************                                                //~1227R~
    ...
    {                                                              //~1224R~
        return graphics;    //Ahsv.Graphics;null if immutable    //~1224R~//~@@@@R~
    }                                                              //~1224R~
    ...
    {                                                              //~1227I~
        if (Dump.C) Dump.println("Image:recycle "+this.toString());//~1506R~//~1A6AR~
        if (bitmap==null)                                          //~1422I~
        	return;                                                //~1422I~
        if (!bitmap.isRecycled())                                  //~1422I~
        {                                                          //~1422I~
         	if (Dump.C) Dump.println("Image:recycle bitmap w="+bitmap.getWidth()+",h="+bitmap.getHeight()+",byte="+getByteCount(bitmap)+"="+bitmap.toString());//~1Ad8I~
    ...
    {                                                              //~1421I~
        ...
        if (Dump.C) Dump.println("Image:createBitmap w="+Pw+",h="+Ph+"byte="+getByteCount(bm)+"="+bm.toString());//~1A4hR~//~1A6AI~
        return bm;
    }                                                              //~1421I~
    public static Image loadPieceImage(int Presid,int Pw,int Ph)  //~@@@@R~
    {                                                              //~@@@@I~
        Bitmap bm=BitmapFactory.decodeResource(AG.resource,Presid);//~@@@@R~
        if (Dump.C) Dump.println("loadPieceImage decode w="+bm.getWidth()+",h="+bm.getHeight()+",bm="+bm.toString());//+1Ad9R~
        Bitmap bmscaled=Bitmap.createScaledBitmap(bm,Pw,Ph,true/*filter*/);       //~@@@@I~
        if (bmscaled!=bm)                                          //~@@@@I~
        {                                                          //~1A6AI~
  	        if (Dump.C) Dump.println("Image:loadPieceImage decoderesource recycle w="+bm.getWidth()+",h="+bm.getHeight()+"byte="+getByteCount(bm)+"="+bm.toString());//~1A4hR~//~1A6AI~//+1Ad9M~
        	bm.recycle();                                          //~@@@@I~
            bm=null;	//for GC ?                                 //~1Ad9R~
        }                                                          //~@@@@I~//~1A6AI~
    	Image image=new Image(Pw,Ph,bmscaled);                     //~@@@@I~
        if (Dump.C) Dump.println("loadPieceImage id="+Integer.toHexString(Presid)+",w="+Pw+",h="+Ph+",bitmap w="+bmscaled.getWidth()+",h="+bmscaled.getHeight()+",bitmap="+bmscaled.toString());//~@@@@R~//~1A6AR~//~1Ad9R~
        return image;                                              //~@@@@R~
    }                                                              //~@@@@I~
    //*******************************************                  //~1A4hR~//~1Ad6I~
    static public int getByteCount(Bitmap Pbm)                     //~1A4hR~//~1Ad6I~
    {                                                              //~1A4hR~//~1Ad6I~
    	if (Pbm==null)                                             //~1A4hR~//~1Ad6I~
        	return 0;                                              //~1A4hR~//~1Ad6I~
        if (AG.osVersion>=AG.HONEYCOMB_MR1)  //android3.1          //~1A4hR~//~1Ad6I~
            return getByteCount_V12(Pbm);                          //~1A4hR~//~1Ad6I~
        return 0;                                                  //~1A4hR~//~1Ad6I~
    }                                                              //~1A4hR~//~1Ad6I~
	@TargetApi(AG.HONEYCOMB_MR1)                                   //~1A4hR~//~1Ad6I~
    static private int getByteCount_V12(Bitmap Pbm)                //~1A4hR~//~1Ad6I~
	{                                                              //~1A4hR~//~1Ad6I~
        return Pbm.getByteCount();                                 //~1A4hR~//~1Ad6I~
    }                                                              //~1A4hR~//~1Ad6I~
}//class                                                           //~1112I~

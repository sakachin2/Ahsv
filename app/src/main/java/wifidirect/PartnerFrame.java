...
//      this(name,serving);                                        //~1A6BI~//+1AecR~
	...
	{	if (Dump.Y) Dump.println("wifidirect:PartnerFrame:connect Starting partner connection");   //~@@@1R~//~1A8cI~
		try
		...
		{                                                          //~v101R~
        	if (Dump.Y) Dump.println("wifidirect.PartnerFrame:connect cause="+e.getCause()+",msg="+e.getMessage());//~1B1jR~//~1A8cI~
            Dump.println(e,"wifidirect.PartnerFrame:connect "+s+",port="+p);  //~1B1jR~//~1A8cI~
			return false;                                          //~v101I~
		}
...
//        show();                                                  //~@@@2R~
	...
	{	if (Dump.Y) Dump.println("wifidirect.PartnerFrame:open Starting partner server");       //~@@@1R~//~1A8cI~
		Server=server;
		try
		{                                                          //~v101R~
            Server.setTcpNoDelay(true);                            //~v101R~
		...
		{	if (Dump.Y) Dump.println("wifidirect.PartnerFrame:open ---> no connection");        //~@@@1R~//~1A8cI~
...
//  	PT=new PartnerThread(In,Out,Input,Output,this);            //~v108R~
    ...
    {                                                              //~v108I~
      try                                                          //~1Ac0I~
      {                                                            //~1Ac0I~
        if (Dump.Y) Dump.println("wifidirect.PartnerFrame:doclose()");        //~1A6BI~//~1A8cI~
		out("@@@@end");                                            //~@@@2R~
//        Out.close();                                             //~1Ac0R~
//        if (Dump.Y) Dump.println("wifidirect.PartnerFrame:doclose issue Out.close()");//~1A6BI~//~1A8cI~//~1Ac0R~
...
//  {	if (s.startsWith("@@name"))                                //~v107R~
	{                                                              //~v107I~
        if (Dump.Y) Dump.println("wifidirect.PartnerFrame:interpret:"+s);     //~v107I~//~1A8cI~
...
//  		setTitle(Global.resourceString("Connection_to_")+name); //~1A6BI~//~1A85R~
    		...
    		setTitle(getConnectionTypeTitleString(ct)+Global.resourceString("Connection_to_")+name);//~1A8iI~//~1A8oR~
        	if (Dump.Y) Dump.println("wifidirect.PartnerFrame:interpret setTitle:"+Global.resourceString("Connection_to_")+name);//~1A6BI~//~1A8cI~//~1A8oR~
        	if (Dump.Y) Dump.println("wifidirect.PartnerFrame:interpret connectiontype="+connectionType);//~1A6BI~//~1A8cI~
		...
		{                                                          //~1A37I~
    		...
    		setTitle(getConnectionTypeTitleString(connectionType)+Global.resourceString("Connection_to_")+PartnerName);//~1A8iI~//~1A8oR~
        	if (Dump.Y) Dump.println("wifidirect.PartnerFrame:interpret receivedWD:partnername="+PartnerName);//~1A6BI~//~1A8aR~//~1A8cI~//~1A8oR~
		}                                                          //~1A37I~
...
//***************************************************************  //~@@@2I~//~1A8oI~
    ...
    {                                                              //~@@@2I~//~1A8oI~
	    ...
	    if (Dump.Y) Dump.println("wifidirect.PartnerFrame:getHostAddr remote="+AG.RemoteInetAddressLAN+",local="+AG.LocalInetAddressLAN);//~1A8fI~//~1A8oI~
    }                                                              //~@@@2I~//~1A8oI~
//***************************************************************  //~1A8iI~
